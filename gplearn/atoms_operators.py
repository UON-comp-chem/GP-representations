...
https://github.com/ulissigroup/GASpy/blob/master/gaspy/atoms_operators.py
We add it here to solve dependency issues.

All credit goes to the original author: 
'Zachary W. Ulissi', 'Kevin Tran'
...
def fingerprint_adslab(atoms, adsorbate = None):
    '''
    ...
    if adsorbate == None:
        ...
        binding_positions = binding_positions[1]
    else: 
        adsan = atomic_numbers[adsorbate]
        adsorbate = atoms.copy()[atoms.get_tags() == 1]
        adsorbate = adsorbate[adsorbate.get_atomic_numbers() == adsan]
        assert len(adsorbate) == 1, "The is designed for single ads site"
        atoms, _ = remove_adsorbate(atoms)
        binding_positions = adsorbate.positions[0]
        
    atoms += Atoms('U', positions=[binding_positions])
